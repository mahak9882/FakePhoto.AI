...
def bezel_border_features(gray, img):
    """Screens are often photographed with visible bezels or frames around
    them — a dark rectangular border surrounding a brighter rectangle.
    This feature measures: (1) the brightness contrast between the
    outermost border strip and the inner region, (2) glare blob count
    (specular highlights from the screen surface), and (3) the spatial
    uniformity of brightness across the image — screens tend to have a
    contained bright zone while real scenes have more continuous gradients.
    """
    h, w = gray.shape
    border = 0.08  # 8% border strip width
    bh, bw = int(h * border), int(w * border)

    inner = gray[bh:h-bh, bw:w-bw].astype(np.float32)
    outer_mask = np.ones_like(gray, dtype=bool)
    outer_mask[bh:h-bh, bw:w-bw] = False
    outer = gray[outer_mask].astype(np.float32)

    inner_mean = inner.mean()
    outer_mean = outer.mean() if outer.size > 0 else inner_mean
    border_contrast = (inner_mean - outer_mean) / (inner_mean + outer_mean + 1e-8)

    # glare/specular blobs: bright, small, localized regions
    bright_mask = (gray.astype(np.float32) > 240).astype(np.uint8)
    num_labels, _, stats, _ = cv2.connectedComponentsWithStats(bright_mask)
    glare_blobs = 0
    for i in range(1, num_labels):
        area = stats[i, cv2.CC_STAT_AREA]
        if 50 < area < 5000:  # medium-sized: too small=noise, too large=overexposure
            glare_blobs += 1

    # spatial brightness non-uniformity: divide image into 3x3 blocks,
    # measure std of block means — screens have a concentrated bright zone
    tile_means = []
    for i in range(3):
        for j in range(3):
            t = gray[i*h//3:(i+1)*h//3, j*w//3:(j+1)*w//3]
            tile_means.append(t.mean())
    spatial_nonuniformity = float(np.std(tile_means))

    return [border_contrast, glare_blobs / 10.0, spatial_nonuniformity / 50.0]


def color_consistency_features(img):
    """Screens emit calibrated RGB that often has tighter color gamut
    consistency than real scenes lit by mixed, spectrally uneven light
    sources. We measure the inter-channel correlation and the blue-channel
    bias (screens, especially phone OLEDs, tend to be slightly cool/blue
    vs. warm ambient light in real photos)."""
    r = img[:,:,0].astype(np.float32).ravel()
    g = img[:,:,1].astype(np.float32).ravel()
    b = img[:,:,2].astype(np.float32).ravel()

    # sample for speed
    idx = np.random.default_rng(0).integers(0, len(r), size=min(10000, len(r)))
    ...
    # blue bias: (mean_b - mean_r) / (mean_b + mean_r)
    blue_bias = float((b.mean() - r.mean()) / (b.mean() + r.mean() + 1e-8))

    return [rg_corr, rb_corr, gb_corr, blue_bias]
...
def extract_features(path):
    ...
    feats += bezel_border_features(gray, img)
    feats += color_consistency_features(img)
...
FEATURE_NAMES = (
    ...
    ["border_contrast", "glare_blobs", "spatial_nonuniformity"] +
    ["rg_corr", "rb_corr", "gb_corr", "blue_bias"]
)
